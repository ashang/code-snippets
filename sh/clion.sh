...
# Try (in order): CL_JDK, clion.jdk, ./jre64, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
if [ -n "$CL_JDK" -a -x "$CL_JDK/bin/java" ]; then
  JDK="$CL_JDK"
fi

if [ -z "$JDK" -a -s "$HOME/.CLion2017.3/config/clion.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.CLion2017.3/config/clion.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
if [ -z "$JDK" -o ! -x "$JAVA_BIN" ]; then
  message "No JDK found. Please validate either CL_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  exit 1
fi
...
# ---------------------------------------------------------------------
if [ -n "$CL_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$CL_PROPERTIES"
fi

VM_OPTIONS_FILE=""
if [ -n "$CL_VM_OPTIONS" -a -r "$CL_VM_OPTIONS" ]; then
  # explicit
  VM_OPTIONS_FILE="$CL_VM_OPTIONS"
elif [ -r "$IDE_HOME.vmoptions" ]; then
  # Toolbox
  VM_OPTIONS_FILE="$IDE_HOME.vmoptions"
elif [ -r "$HOME/.CLion2017.3/config/clion$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.CLion2017.3/config/clion$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/clion$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/clion$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/clion$BITS.vmoptions"
fi
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$AGENT_PATH" ]; then
    AGENT="-agentpath:$AGENT_PATH=probe_disable=*,disablealloc,disabletracing,onlylocal,disableexceptiontelemetry,delay=10000,sessionname=CLion2017.3"
  fi
fi
...
if [ -n "$CL_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$CL_CLASSPATH"
fi
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=CLion2017.3 \
